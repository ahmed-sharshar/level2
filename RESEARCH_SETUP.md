...
2. **Surface points:** check 3–5 real surfaces on each side. Move/add/delete points as needed. Add cross-frame observations only for the same physical patch. Missing projections do not establish invisibility.
...
All included scenes must pass setup checks before publication. Reasoned exclusions allow a pilot subset, but do not exclude difficult scenes to inflate agreement. Episode 38 has only two exterior instance proposals: add a legitimate third surface or exclude it. Ten earlier visual-review flags remain. None of the candidates is automatically human-certified.
