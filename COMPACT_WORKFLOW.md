...
The original compact revision changed presentation and repeated data entry without dropping annotation facts. The 2026-09-21 opening update additionally collects two new human facts, width category and blockers, under `boundary_questions_version: 1`; see [BOUNDARY_QUESTIONS.md](BOUNDARY_QUESTIONS.md) for upgrade rules. The existing `blockmind_l2_annotations_v2` export remains authoritative. The later collection-readiness revision versions the shared tasks, adds three exterior targets and defines shared provisional directions; existing targets, frozen images and open/sealed conditions remain unchanged.
...
incident-ray vector; the written hypothetical scenario defines direction. See `SUN_RAIN.md`.
...
Exports still pass the shared validator, completeness checks and two-rater consensus. Finishing a panel requires its applicable fields, not just visiting it. A remaining blank still blocks completion; Not sure is explicit and requires the existing scene note. Blank or malformed directions still block physics questions. Explicit provisional collection mode permits complete shared scenario proposals without claiming research approval or eligible benchmark GT. Completed records remain locked until reopened.
...
no adjudication is provided. Different unique annotator IDs separate local drafts;
separate browser profiles/devices are optional. IDs are not passwords. See
