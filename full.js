...
(function () {
  ...
  const readOnly = () => state.conflict || ['complete','excluded'].includes(record().status);
  const materialLabel = m => G.canonicalMaterial(m);
  const safeIdentity = value => typeof value==='string' && value.trim() && value.trim().length<=80;
  const panelComplete = p => !p.blocked&&p.questions.every(q=>!q.blocked&&answered(get(record(),q.path)));
  ...
  function renderAll() {renderNavigation();renderFilmstrip();renderFrame();renderPanel();updateProgress();}
  ...
  function goScene(index) {stopPlay();state.episode=Math.max(0,Math.min(state.doc.episodes.length-1,index));state.section='scene';state.question=0;state.frame=0;state.reference=false;state.focusMarker=null;renderAll();save();}
  ...
  function compactControl(q,label,extra='') {
    ...
    if(q.kind==='choice')return '<label class="compact-field" for="'+id+'"><span>'+esc(label||q.title)+'</span><select id="'+id+'" data-field="'+esc(q.path)+'"'+disabled+'><option value=""'+(!answered(value)?' selected':'')+'>Choose…</option>'+q.options.map(o=>'<option value="'+esc(o.value)+'"'+(o.value===value?' selected':'')+'>'+esc(o.label)+'</option>').join('')+'</select>'+extra+'</label>';
  ...
  function updateField(path,value,rerender=false) {
    ...
    set(record(),path,value);if(q.marker_id&&state.section==='exposure'&&state.focusMarker!==q.marker_id)focusPoint(q.marker_id);
  ...
  function reopenScene() {
    if(state.conflict)return;if(layout().disposition==='exclude'){showMessage('Excluded in the shared task package','Only the researcher can change a scene excluded for both annotators.');return;}
  ...
  function changeProfile(value) {
    if(state.conflict){$('profileSelect').value=state.doc.profile;return;}if(value===state.doc.profile)return;
