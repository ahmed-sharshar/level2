...
| Added opening overview | Door/window/open-gap type, human width category, pane transparency, blockers and observed state in one form; versioned preservation of earlier answers | Machine mesh-object span is not certified clear-opening width or a prefilled human size label |
...
| Family B | Agreed transparency/state/glazing/blocker facts; four independent consistency-only passage checklists; separately labelled unapproved rule candidates | Checklists never supply GT. Full rule table, including obscured panes and directional paths, still requires approval; no fabricated transparency quotas |
...
| T14 later out-of-view audit | Ordinary Level 2 adds one images 7–12 visibility set per indoor point; optional Level 2 + Level 3 also records full boundary/per-marker classifications | Direct/through-glass sightings distinguished from reflection-only in instructions; disagreement drops affected visibility item. This does not certify every L3 horizon/full route |
